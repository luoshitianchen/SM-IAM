...
class Settings(BaseSettings):
    ...
    # ── 服务元数据 ──
    VERSION: str = "2.1.0"
    SERVICE_NAME: str = "sm-iam"
    DISPLAY_NAME: str = "SM IAM"
    DESCRIPTION: str = "统一身份认证中心：SSO、OIDC、MFA 与账号生命周期"
    ...
    INTEGRATION_DEPENDENCIES: list[str] = Field(default_factory=lambda: ['sm-audit-log-center'])
    INTEGRATION_EVENTS: list[str] = Field(default_factory=lambda: ['health.checked', 'resource.changed', 'audit.recorded'])

    # ── SM-IAM 特有 ──
    IAM_BOOTSTRAP_USER: str = Field(default="admin")
    IAM_BOOTSTRAP_PASSWORD: str = Field(default="")
    IAM_BOOTSTRAP_CLIENT_ID: str = Field(default="fusion-gateway")
    IAM_BOOTSTRAP_CLIENT_SECRET: str = Field(default="")
